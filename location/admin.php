 ...
 ob_start();
  ...
  switch ($do) {
    ...
    case 'network':
	    ...
	    if ($link_osx)  sqlConfigSet('game-link-osx', $link_osx);
	    if ($link_lin)  sqlConfigSet('game-link-lin', $link_lin);
	if (isset($_POST['new_ver_mcr_stable'])) {
		$ver['ver_mcr_stable']				= InputGet('new_ver_mcr_stable', 'POST', 'str');
		$ver['ver_mcr_dev']			= InputGet('new_ver_mcr_dev', 'POST', 'str');
		$ver['ver_launch']				= InputGet('new_ver_launch', 'POST', 'str');
		$ver['ver_sys']			= InputGet('new_ver_sys', 'POST', 'str');
		$ver['wocauth_legacy_availible']			= InputGet('new_wocauth_legacy_availible', 'POST', 'bool');
	
		if (NetworkConfig::SaveOptions()) $info .= lng('OPTIONS_COMPLETE');
			else $info .= lng('WRITE_FAIL').' ( '.MCR_ROOT.'network.cfg.php )';
	}
	include View::Get('network.html', $st_subdir);
    break;	
    case 'profile':  
	if ($ban_user) {
