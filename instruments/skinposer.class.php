...
Class SPItemLol extends ItemLol {
private $comments;
...
private $user_id;
	...
	public function Rebuild() {
		...
		if (!file_exists($skin_way)) {
			...
			$this->Delete();
			vtxtlog('[Rebuild][skinGenerator2D] SPItemLol ID '.$this->id.' not founded - delete');
		...
		if (!$skin_ratio) {
			...
			$this->Delete();
			vtxtlog('[Rebuild][skinGenerator2D] SPItemLol ID '.$this->id.' wrong skin format - delete');
		...
		if (!skinGenerator2D::savePreview($preview_way, $skin_way, false, false, 160) ) {
			...
			$this->Delete();
			vtxtlog('[Rebuild][skinGenerator2D] Fail to create preview for SPItemLol ID '.$this->id);
		...
		if (!file_exists($preview_way))
			...
			vtxtlog('[Rebuild][skinGenerator2D] Fail to save preview for SPItemLol ID '.$this->id);
...
private $answer;
	...
	public function ShowById($id, $list = 1) {
		...
		$skins_count = 1;		
		$skin = new SPItemLol($id, $this->st_subdir);
	...
	global $user;		
		...
		if (!$skins_count) $html_skin_list = $this->ShowPage('skin_empty.html');
			...
			while ( $line = mysql_fetch_array( $result, MYSQL_NUM ) ) {
				...
				$skin = new SPItemLol($line[0], $this->st_subdir);
