	...
	global $bd_names;	
		...
		if ( mysql_num_rows( $result ) != 0 ) {
		  ...
		  while ( $line = mysql_fetch_array( $result, MYSQL_NUM ) ) {
				...
				$news_item = new News_ItemLol($line[0]);
				$news_item->Delete(); 
				unset($news_item);
		  }
		}
...
Class News_ItemLol extends ItemLol {
private $category_id;
private $discus;
private $title;
private $vote;
...
private $comments;
	...
	global $bd_names, $bd_users;
		...
		$result = BD("SELECT `{$bd_names['users']}`.`{$bd_users['login']}`, `{$this->db}`.`category_id`, `{$this->db}`.`title`, `{$this->db}`.`discus`, `{$this->db}`.`comments`, `{$this->db}`.`vote` FROM `{$this->db}`LEFT JOIN `{$bd_names['users']}` ON `{$this->db}`.user_id = `{$bd_names['users']}`.`{$bd_users['id']}` WHERE `{$this->db}`.`id`='".$this->id."'");
		if ( mysql_num_rows( $result ) != 1 ) { $this->id = false; return false; }
	...
	global $user, $bd_names;
		...
		loadTool('comment.class.php');
		  ...
		  while ( $line = mysql_fetch_array( $result, MYSQL_NUM ) ) {
				...
				$comments_item = new Comments_ItemLol($line[0], false);
...
private $category_id;
		...
		if (isset($_POST['title']) and isset($_POST['message']) and isset($_POST['cid'])) {
			...
			if (empty($_POST['title']) or empty($_POST['message']) or empty($_POST['cid'])) 
				...
				if ($editMode > 0) {
				    ...
				    $news_item = new News_ItemLol($editMode, $this->st_subdir);
		    ...
		    $news_item = new News_ItemLol((int)$_GET['delete']);
			$news_item->Delete();
			...
			$news_item = new News_ItemLol((int)$_GET['edit']);
	...
	if ( mysql_num_rows( $result ) != 0 ) {
		  ...
		  while ( $line = mysql_fetch_array( $result , MYSQL_NUM ) ) {
		         ...
		         $news_item = new News_ItemLol($line[0], $this->st_subdir);
