...
private $verified;
	...
	global $bd_users, $bd_names;	
		...
		$sql = "SELECT * FROM `{$this->db}` WHERE `".TextBase::SQLSafe($method)."`='".TextBase::SQLSafe($input)."'";
		...
		$this->verified   = (boolean)$line['verified'];
	...
	public function changeName($newname, $force = false) {
	global $bd_users, $site_ways;
	
		if (!$this->id) return 0;
		if($this->name == $newname) return 1;
		$newname = trim($newname);
		
		if (!preg_match("/^[a-zA-Z0-9_-]+$/", $newname)) return 1401;
		
		if($force) BD("DELETE FROM `{$this->db}` WHERE `{$bd_users['login']}`='".TextBase::SQLSafe($newname)."';");
	...
	global $bd_users, $bd_names;	
		...
		if ( mysql_num_rows( $result ) != 0 ) {
		  ...
		  while ( $line = mysql_fetch_array( $result, MYSQL_NUM ) ) {
				...
				$comment_del = new Comments_ItemLol($line[0]);
				$comment_del->Delete(); 
				unset($comment_del);
		  }
		}
